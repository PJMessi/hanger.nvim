...
local function get_test_at_cursor()
	...
	if
		not node
		or (node:type() == "function_item" and not is_test_func(node))
		or (node:type() == "mod_item" and not is_test_mod(node))
	then
		return nil, nil
	end
...
local function build_cmd(test_name, mods)
	...
	local cmd = string.format("cargo test --package %s --bin %s -- %s", package_name, package_name, test_pattern)
